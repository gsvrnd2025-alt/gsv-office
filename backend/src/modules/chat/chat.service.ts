...
export class ChatService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      // Ensure 'folder' and 'smb_folder' exist in the message_type enum
      ...
      const hasSmbFolder = types.some((t: any) => t.enumlabel === 'smb_folder');
      if (!hasSmbFolder) {
        await this.dataSource.query(`ALTER TYPE message_type ADD VALUE 'smb_folder';`);
      }
    } catch (err) {
  ...
  async getMessages(conversationId: string, userId: string, page = 1, limit = 500) {
    return this.dataSource.query(`
      SELECT m.id, m.conversation_id, m.sender_id, m.content, m.file_id, m.folder_id, m.reply_to_id, m.metadata, m.created_at, m.deleted_at,
             CASE
      ...
      GROUP BY m.id, m.conversation_id, m.sender_id, m.content, m.file_id, m.folder_id, m.reply_to_id, m.metadata, m.created_at, m.deleted_at, 
             CASE
               WHEN m.type::text = 'image' THEN 'photo'
               WHEN m.type::text = 'audio' THEN 'music'
               ELSE m.type::text
  ...
  async sendMessage(dto: { conversationId: string; senderId: string; content: string; type?: string; fileId?: string; folderId?: string; replyToId?: string; metadata?: any }) {
    let mappedType = dto.type || 'text';
    if (mappedType === 'photo') mappedType = 'image';
    if (mappedType === 'music') mappedType = 'audio';

    const validTypes = ['text', 'image', 'video', 'audio', 'document', 'file', 'voice_note', 'system', 'folder', 'smb_folder'];
    ...
    const metadata = dto.metadata ? (typeof dto.metadata === 'string' ? dto.metadata : JSON.stringify(dto.metadata)) : '{}';

    const [msg] = await this.dataSource.query(
      `INSERT INTO messages (conversation_id, sender_id, content, type, file_id, folder_id, reply_to_id, metadata)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [dto.conversationId, dto.senderId, dto.content, mappedType, fileId, folderId, replyToId, metadata]
    );
